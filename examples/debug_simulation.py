"""
examples/debug_simulation.py

Issue #16: This diagnostic script was originally at
``tests/debug_simulation.py`` where pytest picked it up
as a test function (``test_debug_simulation``).  It
contains only ``print()`` statements and no assertions,
so it is a diagnostic script masquerading as a test.
It has been moved to ``examples/`` and the ``test_``
prefix removed so pytest no longer treats it as a test.

Run it directly::

    python examples/debug_simulation.py

or::

    python -m examples.debug_simulation
"""
...
def run_debug_simulation(
    ...
    """
    Diagnostic script — prints simulation details.

    This is NOT a test; it is a developer debugging aid.
    """
...
if __name__ == "__main__":
    ...
    # When run directly, a CSV path must be supplied.
    import sys

    if len(sys.argv) < 2:
        print(
            "Usage: python examples/debug_simulation.py "
            "<path-to-csv>"
        )
        sys.exit(1)
