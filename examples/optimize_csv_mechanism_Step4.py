"""
examples/optimize_csv_mechanism_Step4.py

Schritt 4: Zwei verkettete Getriebestufen, die ausserhalb des
relevanten Bereichs blockieren koennen

User I/O angles are in DEGREES.
Internal calculations use RADIANS.

Dieses Beispiel ist das Analogon zu Schritt 2, jedoch mit zwei
verketteten Getriebestufen statt einer. Es verwendet einen
Mechanismus mit drei Hebeln:
- Eingangshebel (Hebel 1): Drehpunkt bei (0,0,0), Laenge 100mm,
  Winkelbereich [-51, 51]
- Zwischenhebel (Hebel 2): Drehpunkt bei (100,0,0), Laenge 100mm,
  Winkelbereich [-50, 50], getrieben von Hebel 1
- Ausgangshebel (Hebel 3): Drehpunkt bei (200,0,0), Laenge 100mm,
  Winkelbereich [-49, 49], getrieben von Hebel 2

Beide Stufen bilden je einen Rhombus: In jeder Stufe entspricht die
Stangenlaenge (100mm) genau dem Abstand der beiden Drehpunkte (100mm).
Dadurch ist die Geometrie innerhalb des relevanten Bereichs frei
beweglich. Jede Stufe hat jedoch einen Ausgangshebel, dessen
Winkelbereich etwas schmaler ist als der Bereich des zugehoerigen
Eingangshebels (Stufe 1: [-51, 51] -> [-50, 50],
Stufe 2: [-50, 50] -> [-49, 49]). Dies fuehrt dazu, dass beide
Stufen bei Eingangswinkeln ausserhalb des relevanten Bereichs
[-45, 45] blockieren KOENNEN, sobald der jeweilige Ausgangshebel
seinen maximalen Ausschlag erreicht. Innerhalb des relevanten
Bereichs [-45, 45] laufen beide Stufen frei. Die Einschraenkung
ist absichtlich knapp gewaehlt (jeweils nur 1 Grad), damit der
nicht-blockierende Bereich breit genug bleibt, damit der
Optimierer die Loesung selbst finden kann.

Die Zielkurve ist eine einfache 1:1 Uebersetzung von -45 bis +45 Grad.
Jede Stufe uebersetzt ihren Eingang 1:1 auf ihren Ausgang, so dass die
gesamte Kette ebenfalls eine 1:1 Uebersetzung ueber den Bereich
[-45, 45] realisiert.
"""
...
MECHANISM_FILE = BASE_DIR / "mechanism_Step4.csv"
TARGET_FILE = BASE_DIR / "target_curve_Step4.csv"
...
print("=" * 80)
print("SCHRITT 4: ZWEI VERKETTETE STUFEN, BLOCKIEREND AUSSERHALB [-45, 45]")
print("=" * 80)
...
print(f"\nStufen: {len(definition.levers) - 1}")
print("  Stufe 1: Hebel 1 -> Hebel 2 (Rhombus, Ausgang eingeschraenkt [-50, 50])")
print("  Stufe 2: Hebel 2 -> Hebel 3 (Rhombus, Ausgang eingeschraenkt [-49, 49])")
print("\nIn jeder Stufe entspricht die Stangenlaenge (100mm) genau dem Abstand")
print("der Drehpunkte (100mm). Der Ausgangshebel jeder Stufe ist jeweils 1 Grad")
print("schmaler als sein Eingangshebel (Stufe 1: [-51, 51] -> [-50, 50],")
print("Stufe 2: [-50, 50] -> [-49, 49]). Dadurch koennen beide Stufen bei")
print("Eingangswinkeln ausserhalb des relevanten Bereichs [-45, 45] blockieren,")
print("laufen innerhalb von [-45, 45] aber frei. Die Einschraenkung ist knapp")
print("gewaehlt, damit der nicht-blockierende Bereich breit genug fuer den")
print("Optimierer bleibt.")
...
# -------------------------------------------------
#
# Wie bei Schritt 3 werden zwei verkettete Getriebestufen
# optimiert. In jeder Stufe entspricht die Stangenlaenge
# genau dem Drehpunktabstand, so dass nur eine schmale
# geometrische Konfiguration nicht blockiert. Bei zwei
# verketteten, den Bereich einschraenkenden Stufen (analog
# Schritt 2) ist der Bereich nicht-blockierender Geometrien
# schmaler als bei den nie blockierenden Stufen aus
# Schritt 3: in jeder Stufe muss die Referenzgeometrie auf
# der jeweiligen 1:1-Diagonalen liegen. Damit der
# evolutionaere Algorithmus diese Konfiguration selbst
# findet (ohne Vorlage einer Loesung), werden die
# Ausgangshebelbereiche nur knapp eingeschraenkt (jeweils
# 1 Grad schmaler als der Eingangshebel). Dadurch bleibt
# der nicht-blockierende Bereich breit genug, dass der
# Optimierer zuverlaessig selbst eine nicht-blockierende
# Loesung findet, die der 1:1 Zielkurve entspricht und
# innerhalb [-45, 45] frei laeuft. Mit einer
# Startpopulation von 600 Kandidaten konvergiert die
# Evolution reproduzierbar.
# -------------------------------------------------

rng = random.Random(42)  # Fixed seed for reproducibility
population_factory = PopulationFactory(random_generator=rng)
population = population_factory.create(parameter_template, size=600)
...
for generation in engine.run(children_count=600):
    generation_count += 1
...
print("=" * 80)
print("\nHinweis: Dieser Mechanismus besteht aus drei Hebeln der Laenge 100mm,")
print("die als zwei verkettete Rhomben angeordnet sind. In jeder Stufe betraegt")
print("der Abstand der Drehpunkte 100mm und entspricht damit genau der")
print("Stangenlaenge. Der Ausgangshebel jeder Stufe ist jeweils 1 Grad")
print("schmaler als sein Eingangshebel (Stufe 1: [-51, 51] -> [-50, 50],")
print("Stufe 2: [-50, 50] -> [-49, 49]).")
print("\nDadurch koennen beide Stufen bei Eingangswinkeln ausserhalb des")
print("relevanten Bereichs [-45, 45] blockieren, sobald der jeweilige")
print("Ausgangshebel seinen maximalen Ausschlag erreicht. Innerhalb des")
print("relevanten Bereichs [-45, 45] laufen beide Stufen frei und realisieren")
print("zusammen eine 1:1 Uebersetzung. Die Einschraenkung ist absichtlich")
print("knapp gewaehlt, damit der Optimierer die Loesung selbst finden kann.")
print("\nVergleich mit den vorherigen Schritten:")
print("  Schritt 1: Eine Stufe, nie blockierend (Rhombus)")
print("  Schritt 2: Eine Stufe, blockiert ausserhalb des relevanten Bereichs")
print("  Schritt 3: Zwei verkettete Stufen, beide nie blockierend (Rhombus)")
print("  Schritt 4: Zwei verkettete Stufen, beide blockieren ausserhalb [-45, 45]")
print("=" * 80)
